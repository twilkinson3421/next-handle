  ...
  export namespace Service {
    ...
    >(action: () => ActionReturn) {
      try {
        ...
        if (response.detail) {
          const method: keyof typeof konsole = response.ok ? "success" : "err";

          const THIS_IS_LOGGED = LOG_STATUS_ARR?.includes(
            response.status.toString()
          );
          const CATCH_ALL_LOG = response.ok ? LOG_ALL_OK : LOG_ALL_ERR;

          const THIS_IS_EXCLUDED = EXCLUDE_STATUS_ARR?.includes(
            response.status.toString()
          );
